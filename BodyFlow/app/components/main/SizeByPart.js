                        ...
                        <TouchableOpacity style={styles.box} onPress={() => this.onPress(part)}>
                            ...
                            <View style={common.textBoxEnd}>
                                {/* 기본 단위는 cm. inch로 변환해야할 경우 변환하여 렌더링 */
                                    this.props.unit == 'cm' ?
                                        <Text style={styles.size}>{this.state.sizeParts[part] == null ? '0.0' : this.state.sizeParts[part]}</Text>
                                        : <Text style={styles.size}>{this.state.sizeParts[part] == null ? '0.0' : (this.state.sizeParts[part] / 2.54).toFixed(2)}</Text>
                                }
